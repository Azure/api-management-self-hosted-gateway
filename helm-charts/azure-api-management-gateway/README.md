...
![Version: 1.15.1](https://img.shields.io/badge/Version-1.15.1-informational?style=flat-square) ![AppVersion: 2.11.1](https://img.shields.io/badge/AppVersion-2.11.1-informational?style=flat-square) [![Documentation](https://img.shields.io/badge/docs-azure.com-blue?style=flat-square)](https://docs.microsoft.com/azure/api-management/self-hosted-gateway-overview)
...
| `observability.logs.std.level` | string | `"info"` | Level of logs outputted to standard output. Allowed values are `all`, `debug`, `info`, `warn`, `error` or `fatal`. Severity increases from `all` (everything) and `debug` (debug and above) through `info`, `warn`, and `error`, to `fatal` (fatal events only). Learn more in [our documentation](https://docs.microsoft.com/azure/api-management/how-to-configure-local-metrics-logs#logs). |
