...
> **⚠️ This README is auto-generated using [helm-docs](https://github.com/norwoodj/helm-docs).**
>
> **DO NOT EDIT THIS FILE DIRECTLY.** To update documentation, modify comments in `values.yaml` or edit `README.md.gotmpl`, then run `helm-docs` in this directory.
...
#### Production Configuration with HA and Observability
...
This README is automatically generated from the Helm chart's metadata and values file. To update the documentation:

1. **Update value descriptions**: Modify the `# --` comments in `values.yaml` to improve the configuration table.
2. **Update README structure**: Edit `README.md.gotmpl` to change sections, add examples, or modify the layout.
3. **Regenerate README**: Run `helm-docs` in this directory to regenerate `README.md`.
4. **Commit changes**: Commit both your changes to `values.yaml`/`README.md.gotmpl` AND the regenerated `README.md`.

For detailed contributing guidelines, see [CONTRIBUTING.md](../../CONTRIBUTING.md) in the repository root.

The CI workflow will validate that the README is up-to-date with the values file.
